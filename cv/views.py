...
def serve_protected_file(request, file_type, model_id, field_name):
    ...
    if not file_name:
        # Algunos storages levantan excepción al acceder .url si no hay
        try:
            file_name = getattr(file_field, "url", "") or ""
        except Exception:
            file_name = ""
    ...
    # Fallback local cuando Azure está desactivado
    if not getattr(settings, "USE_AZURE_MEDIA", False):
        try:
            # Abrir el archivo desde el storage por defecto (FileSystemStorage)
            file_field.open("rb")
            content = file_field.read()
            file_field.close()

            filename = os.path.basename(str(file_name).replace("\\", "/").split("?")[0])
            content_type, _ = mimetypes.guess_type(filename)
            if not content_type:
                content_type = "application/octet-stream"

            # Decisión de inline/attachment basada en tipo y extensión
            ext = os.path.splitext(filename)[1].lower()
            inline_exts = {".pdf", ".jpg", ".jpeg", ".png", ".gif", ".webp", ".svg"}
            inline_types = {"application/pdf", "image/jpeg", "image/png", "image/gif", "image/webp", "image/svg+xml"}
            inline_ok = (content_type in inline_types) or (ext in inline_exts)
            disposition = "attachment" if force_download else ("inline" if inline_ok else "attachment")
            ...
            raise Http404("Archivo no encontrado en el almacenamiento local")
        except Exception as e:
            raise Http404(f"Error al leer el archivo local: {str(e)}")

    # Azure habilitado: usar proxy
    try:
        content, blob_properties = AzureFileProxy.download_blob(file_name)

        content_type = getattr(blob_properties.content_settings, "content_type", None)
        if not content_type:
            filename_guess = os.path.basename(str(file_name))
        ...
        raise Http404("Archivo no encontrado en Azure")
    except Exception as e:
